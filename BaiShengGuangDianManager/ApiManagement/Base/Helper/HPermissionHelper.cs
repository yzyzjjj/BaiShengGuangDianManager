...
{
    public class HPermissionHelper
    {
        ...
        {
            PermissionsList = ServerConfig.ApiDb.Query<Permission>("SELECT * FROM `permissions_group` WHERE IsDelete = 0;").ToDictionary(x => x.Id);
            PermissionsDetailList = ServerConfig.ApiDb.Query<Permission>("SELECT * FROM `permissions` WHERE IsDelete = 0;").ToDictionary(x => x.Id);
            //UpdateOrder();
        }
        ...
        {
            ServerConfig.ApiDb.Execute("UPDATE permissions_group SET  `IsDelete` = 1 WHERE `Id` IN @Id AND Type != 0;", new { Id = list });
        }
