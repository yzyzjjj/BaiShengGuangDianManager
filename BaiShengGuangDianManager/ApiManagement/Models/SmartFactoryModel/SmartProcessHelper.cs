...
namespace ApiManagement.Models.SmartFactoryModel
{
    public class SmartProcessHelper : DataHelper
    {
        private SmartProcessHelper()
        {
            Table = "t_process";
            InsertSql =
                "INSERT INTO `t_process` (`CreateUserId`, `MarkedDateTime`, `Process`, `DeviceCategoryId`, `Remark`) " +
                "VALUES (@CreateUserId, @MarkedDateTime, @Process, @DeviceCategoryId, @Remark);";
            UpdateSql =
                "UPDATE `t_process` SET `MarkedDateTime` = @MarkedDateTime, `Process` = @Process, `DeviceCategoryId` = @DeviceCategoryId, `Remark` = @Remark WHERE `Id` = @Id;";
        }
