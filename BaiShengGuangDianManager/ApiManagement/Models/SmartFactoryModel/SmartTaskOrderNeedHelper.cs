...
namespace ApiManagement.Models.SmartFactoryModel
{
    public class SmartTaskOrderNeedHelper : DataHelper
    {
        private SmartTaskOrderNeedHelper()
        {
            ...
            InsertSql =
                "INSERT INTO `t_task_order_need` (`CreateUserId`, `MarkedDateTime`, `Batch`, `WorkshopId`, `TaskOrderId`, `ProcessId`, `PId`, " +
                "`ProductId`, `Target`, `Stock`, `DoneTarget`, `Put`, `HavePut`, `FirstArrangedTime`, `EstimatedStartTime`, `EstimatedEndTime`) " +
                "VALUES (@CreateUserId, @MarkedDateTime, @Batch, @WorkshopId, @TaskOrderId, @ProcessId, @PId, " +
                "@ProductId, @Target, @Stock, @DoneTarget, @Put, @HavePut, @FirstArrangedTime, @EstimatedStartTime, @EstimatedEndTime);";
        ...
        #region Get
        //public static IEnumerable<SmartTaskOrderNeed> GetSmartTaskOrderNeedsByTaskOrderId(int wId, int taskOrderId)
        //{
        ...
        /// <returns></returns>
        public static IEnumerable<SmartTaskOrderNeedDetail> GetSmartTaskOrderNeedsByTaskOrderIds(int wId, IEnumerable<int> taskOrderIds, bool detail = false)
        {
            return ServerConfig.ApiDb.Query<SmartTaskOrderNeedDetail>(!detail ?
                ...
                "FROM (SELECT * FROM (SELECT * FROM `t_task_order_need` WHERE TaskOrderId IN @taskOrderIds AND WorkshopId = @wId ORDER BY TaskOrderId, Batch DESC) a GROUP BY TaskOrderId, PId) a " +
                ...
                "JOIN `t_product` d ON a.ProductId = d.Id GROUP BY TaskOrderId, PId;", new { wId, taskOrderIds });
        }
