...
namespace ApiManagement.Models.AccountManagementModel
{
    ...
    public class EmailHelper
    {
        ...
        {
            return ServerConfig.ApiDb.Query<MailType>("SELECT * FROM `email_type`;");
        }
        ...
        public static void Send(string subject, string body, List<string> emailAddress, int mailType)
        {
            ...
            //emailAddress.AddRange(ServerConfig.ApiDb.Query<string>("SELECT EmailAddress FROM `accounts` WHERE IsDeleted = 0 AND FIND_IN_SET(@EmailType, EmailType)", new
            //{
            //    EmailType = mailType
