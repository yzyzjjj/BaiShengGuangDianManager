...
namespace ApiManagement.Controllers.ProcessManagementController
{
    /// <summary>
    /// 工艺编号
    /// </summary>
    ...
    public class ProcessManagementController : ControllerBase
    {
        ...
        public DataResult GetProcessManagementByProcessNumber()
        {
            ...
            if (processManagements.Any())
            {
                ...
                var productionProcessLibraries = ServerConfig.ApiDb.Query<Production>("SELECT `Id`, ProductionProcessName FROM `production_library` WHERE MarkedDelete = 0;").ToDictionary(x => x.Id);
        ...
        public Result PutProcessManagement([FromBody] ProcessManagement processManagement)
        {
            ...
            try
            {
                ...
                Dictionary<int, Production> productionProcessLibraries = new Dictionary<int, Production>();
                IEnumerable<int> productModelList = null;
                if (!processManagement.ProductModels.IsNullOrEmpty())
                {
                    productionProcessLibraries = ServerConfig.ApiDb.Query<Production>("SELECT `Id` FROM `production_library` WHERE MarkedDelete = 0;").ToDictionary(x => x.Id);
        ...
        public Result PutProcessManagement([FromRoute] string processNumber, [FromBody] Models.ProcessManagementModel.ProcessManagement processManagement)
        {
            var data =
                ServerConfig.ApiDb.Query<Models.ProcessManagementModel.ProcessManagement>("SELECT * FROM `process_management` WHERE `ProcessNumber` = @processNumber AND MarkedDelete = 0;", new { processNumber }).FirstOrDefault();
            if (data == null)
            {
                return Result.GenError<DataResult>(Error.ProcessManagementNotExist);
            }
            ...
            try
            {
                ...
                if (!processManagement.ProductModels.IsNullOrEmpty())
                {
                    var productionProcessLibraries = ServerConfig.ApiDb.Query<Production>("SELECT `Id` FROM `production_library` WHERE MarkedDelete = 0;").ToDictionary(x => x.Id);
                    var productModelList = processManagement.ProductModels.Split(',').Select(int.Parse);
                    if (productModelList.Any(x => !productionProcessLibraries.ContainsKey(x)))
                    {
                        return Result.GenError<Result>(Error.ProductionLibraryNotExist);
