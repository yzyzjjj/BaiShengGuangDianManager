...
namespace ApiManagement.Controllers
{
    ...
    public class UsuallyDictionaryController : ControllerBase
    {
        ...
        public DataResult GetUsuallyDictionary([FromRoute] int id)
        {
            var result = new DataResult();
            var data =
                ServerConfig.ApiDb.Query<UsuallyDictionary>("SELECT * FROM `usually_dictionary` WHERE Id = @id AND `MarkedDelete` = 0;", new { id }).FirstOrDefault();
            if (data == null)
            {
        ...
        public Result PutUsuallyDictionary([FromRoute] int id, [FromBody] UsuallyDictionary usuallyDictionary)
        {
            ...
            ServerConfig.ApiDb.Execute(
                "UPDATE data_name_dictionary SET `MarkedDateTime` = @MarkedDateTime, `MarkedDelete` = @MarkedDelete, `ModifyId` = @ModifyId, `ScriptId` = @ScriptId, " +
