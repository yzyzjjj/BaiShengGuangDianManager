...
namespace ApiManagement.Controllers.DeviceManagementController
{
    ...
    public class SurveyorController : ControllerBase
    {
        // GET: api/Surveyor
        [HttpGet]
        public DataResult GetSurveyor([FromQuery]string ids)
        {
            var result = new DataResult();
            result.datas.AddRange(ServerConfig.ApiDb.Query<Surveyor>($"SELECT * FROM `surveyor` WHERE MarkedDelete = 0{(ids.IsNullOrEmpty() ? "" : " FIND_IN_SET(Id, @ids)")};", new { ids }));
            return result;
        }
        /// <summary>
        /// 自增Id
