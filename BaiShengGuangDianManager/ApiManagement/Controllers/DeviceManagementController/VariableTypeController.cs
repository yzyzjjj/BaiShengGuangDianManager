...
namespace ApiManagement.Controllers.DeviceManagementController
{
    ...
    public class VariableTypeController : ControllerBase
    {
        ...
        public DataResult GetVariableType()
        {
            ...
            result.datas.AddRange(ServerConfig.ApiDb.Query<VariableType>("SELECT * FROM `variable_type` WHERE `MarkedDelete` = 0;"));
            return result;
        }
        ...
        public DataResult GetVariableType([FromRoute] int id)
        {
            var result = new DataResult();
            var data =
                ServerConfig.ApiDb.Query<VariableType>("SELECT * FROM `variable_type` WHERE Id = @id AND `MarkedDelete` = 0;", new { id }).FirstOrDefault();
            if (data == null)
            {
        ...
        {
            var cnt =
                ServerConfig.ApiDb.Query<int>("SELECT COUNT(1) FROM `variable_type` WHERE Id = @id AND `MarkedDelete` = 0;", new { id }).FirstOrDefault();
            if (cnt == 0)
            {
            ...
            ServerConfig.ApiDb.Execute(
                "UPDATE variable_type SET `MarkedDateTime` = @MarkedDateTime, `MarkedDelete` = @MarkedDelete, `ModifyId` = @ModifyId, " +
