...
namespace ApiFlowCardManagement.Controllers
{
    ...
    public class ProcessorController : ControllerBase
    {
        ...
        public DataResult GetProcessor()
        {
            ...
            result.datas.AddRange(ServerConfig.FlowcardDb.Query<Processor>("SELECT * FROM `processor`;"));
            return result;
        }
        ...
        public DataResult GetProcessor([FromRoute] int id)
        {
            var result = new DataResult();
            var data =
                ServerConfig.FlowcardDb.Query<Processor>("SELECT * FROM `processor` WHERE Id = @id;", new { id }).FirstOrDefault();
            if (data == null)
            {
        ...
        {
            var cnt =
                ServerConfig.FlowcardDb.Query<int>("SELECT COUNT(1) FROM `processor` WHERE Id = @id;", new { id }).FirstOrDefault();
            if (cnt == 0)
            {
