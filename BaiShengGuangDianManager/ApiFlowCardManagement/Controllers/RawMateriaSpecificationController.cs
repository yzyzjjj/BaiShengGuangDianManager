...
namespace ApiFlowCardManagement.Controllers
{
    ...
    public class RawMateriaSpecificationController : ControllerBase
    {
        ...
        public DataResult GetRawMateriaSpecification()
        {
            ...
            result.datas.AddRange(ServerConfig.FlowcardDb.Query<dynamic>("SELECT a.*, b.RawMateriaName FROM `raw_materia_specification` a JOIN `raw_materia` b ON a.RawMateriaId = b.Id;"));
            return result;
        }
        ...
        public DataResult GetRawMateriaSpecification([FromRoute] int id)
        {
            var result = new DataResult();
            var data =
                ServerConfig.FlowcardDb.Query<dynamic>("SELECT a.*, b.RawMateriaName FROM `raw_materia_specification` a JOIN `raw_materia` b ON a.RawMateriaId = b.Id WHERE a.Id = @id;", new { id }).FirstOrDefault();
            if (data == null)
            {
        ...
        public DataResult GetRawMateriaSpecification([FromRoute] string rawMateriaName)
        {
            ...
            var result = new DataResult();
            var data =
                ServerConfig.FlowcardDb.Query<dynamic>("SELECT a.*, b.RawMateriaName FROM `raw_materia_specification` a JOIN `raw_materia` b ON a.RawMateriaId = b.Id WHERE a.RawMateriaId = @Id;", new { rawMateria.Id });
            result.datas.Add(data);
            return result;
        }
        ...
        {
            var cnt =
                ServerConfig.FlowcardDb.Query<int>("SELECT COUNT(1) FROM `raw_materia_specification` WHERE Id = @id;", new { id }).FirstOrDefault();
            if (cnt == 0)
            {
            ...
            cnt =
                ServerConfig.FlowcardDb.Query<int>("SELECT COUNT(1) FROM `raw_materia` WHERE Id = @RawMateriaId;", new { rawMateriaSpecification.RawMateriaId }).FirstOrDefault();
            if (cnt == 0)
            {
                return Result.GenError<DataResult>(Error.RawMateriaNotExist);
            }
        ...
        public Result PostRawMateriaSpecification([FromBody] List<RawMateriaSpecification> rawMateriaSpecifications)
        {
            ...
            var cnt =
                ServerConfig.FlowcardDb.Query<int>("SELECT COUNT(1) FROM `raw_materia` WHERE Id IN @rawMateriaIds;", new
                {
