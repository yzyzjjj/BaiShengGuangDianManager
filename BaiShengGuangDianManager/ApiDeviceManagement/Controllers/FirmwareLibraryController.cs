...
namespace ApiDeviceManagement.Controllers
{
    ...
    public class FirmwareLibraryController : ControllerBase
    {
        ...
        public DataResult GetFirmwareLibrary()
        {
            ...
            result.datas.AddRange(ServerConfig.DeviceDb.Query<FirmwareLibrary>("SELECT * FROM `firmware_library` WHERE `MarkedDelete` = 0;"));
            return result;
        }
        ...
        public DataResult GetFirmwareLibrary([FromRoute] int id)
        {
            var result = new DataResult();
            var data =
                ServerConfig.DeviceDb.Query<FirmwareLibrary>("SELECT * FROM `firmware_library` WHERE Id = @id AND `MarkedDelete` = 0;", new { id }).FirstOrDefault();
            if (data == null)
            {
