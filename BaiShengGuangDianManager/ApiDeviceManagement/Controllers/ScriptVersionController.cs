...
namespace ApiDeviceManagement.Controllers
{
    ...
    public class ScriptVersionController : ControllerBase
    {
        ...
        public DataResult GetScriptVersion([FromRoute] int id)
        {
            var result = new DataResult();
            var data =
                ServerConfig.DeviceDb.Query<ScriptVersion>("SELECT * FROM `script_version` WHERE Id = @id AND `MarkedDelete` = 0;", new { id }).FirstOrDefault();
            if (data == null)
            {
        ...
        {
            var result = new DataResult();
            result.datas.AddRange(ServerConfig.DeviceDb.Query<ScriptVersion>("SELECT * FROM `script_version` WHERE DeviceModelId = @deviceModelId AND `MarkedDelete` = 0;", new { deviceModelId }));
            return result;
        }
